...
const GridProductData = () => {
  ...
  for (const data of productData) {
    ...
    content.push(
      /* <div key={data.id} className="flex flex-wrap flex-col items-center">*/
      <div
        key={data.id}
        className="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden
        flex flex-wrap w-full md:flex-col items-center"
      >
        <button
          onClick={() => {
            navigate(path);
        ...
        >
          <img
            className="w-80 h-56 block rounded-lg object-cover object-center"
            alt={data.name}
            src={data.path}
          />
        </button>
